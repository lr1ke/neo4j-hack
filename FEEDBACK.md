...
- Didn't get to the Aura MCP piece in the time available — would have liked to expose these six queries as MCP tools for live/interactive querying during the demo instead of a scripted run. That's the natural next step if there's a round 2.
